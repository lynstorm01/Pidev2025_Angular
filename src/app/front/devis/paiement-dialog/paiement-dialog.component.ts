...
export class PaiementDialogComponent {
  ...
  method: string = 'Carte Bancaire'; // Exemple de méthode de paiement par défaut
  paiementUnique: boolean = true; // Par défaut, paiement en une seule fois
  nombreEcheances: number = 3; // Valeur par défaut si paiement échelonné
  devis: any; // Stocker l'objet devis reçu
  constructor(public dialogRef: MatDialogRef<PaiementDialogComponent>,
    @Inject(MAT_DIALOG_DATA) public data: any,
   private paiementService: PaiementService,
       private snackBar: MatSnackBar
  ) {
    this.devis = data.devis; // Assigner le devis reçu
    console.log("Devis reçu dans le popup:", this.devis); // DEBUG
...
export class PaiementDialogComponent implements AfterViewChecked {
  ...
  pay() {
    ...
    this.http.post<{ clientSecret: string }>('http://localhost:8080/api/payments/create-payment-intent', { amount })
      .subscribe(response => {
