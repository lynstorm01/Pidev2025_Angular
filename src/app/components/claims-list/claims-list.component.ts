  ...
  export class ClaimsListComponent implements OnInit {
    ...
    selectedAnalysis: string | null = null;
    ...
    showAnalysis(claimId: number): void {
      this.claimsService.getClaimAnalysis(claimId).subscribe(
        (result: string) => {
          this.selectedAnalysis = result;
          // Ou utilise un modal/toast/zone dédiée pour afficher ça
        },
        error => {
          console.error('Erreur lors de l’analyse de la réclamation', error);
          this.selectedAnalysis = 'Erreur lors de l’analyse.';
        }
      );
    }
